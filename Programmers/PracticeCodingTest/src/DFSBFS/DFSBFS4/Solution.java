...
class Solution {
    ...
    static void dfsSol(String now, String nodes, int count, String[][] tickets) {
        ...
        for(int i = 0; i < tickets.length; i++) {
            ...
            if(!ticketUsed[i] && tickets[i][0].equals(now)) {
                // 티켓을 사용하고 이동
                ticketUsed[i] = true;
                dfsSol(tickets[i][1], nodes + " " + tickets[i][1],count+1, tickets);
                ticketUsed[i] = false;
            }
        }
    }
